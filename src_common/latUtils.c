...
/**
 * @brief Utilities for an Platon Hardware Wallet logic
 * @file latUtils.h
...
void getLatAddressFromKey(cx_ecfp_public_key_t *publicKey, uint8_t *out, cx_sha3_t *sha3Context) {
...
void getLatAddressStringFromKey(cx_ecfp_public_key_t *publicKey,
                                uint8_t *out,
    ...
    cx_keccak_init(sha3Context, 256);
    cx_hash((cx_hash_t *) sha3Context, CX_LAST, publicKey->W + 1, 64, hashAddress, 32);
    getLatAddressStringFromBinary(hashAddress + 12, out, sha3Context, chain_config);
}

void getLatAddressStringFromBinary(uint8_t *address,
                                   uint8_t *out,
                                   cx_sha3_t *sha3Context,
                                   chain_config_t *chain_config) {
    ...
    // get bech32 address
    encode(address, chain_config->hrp, (char *)out);
